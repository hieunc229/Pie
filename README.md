...
- The box is exactly as wide as the transcript's content column (816pt, or the pane minus a 22pt gutter when there is less room) and starts at the same x as the rows above it, so it reads as part of the conversation rather than as a pane-wide bar. A 16pt margin keeps it off the bottom edge.

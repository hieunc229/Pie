...
- The repeating quiet steps fold instead of stacking: neighbouring `thinking` blocks, `bash` calls and `edit`/`write`/`read` calls become one dimmed, collapsed line — `Run command`, `Edited files`, `Read files` or `Thinking` for a lone step, `Thinking, edited files, run commands` for a run — showing the command or path beside a call (reasoning is named by its family, never previewed). The line itself is the control, with no chevron: clicking anywhere along it opens it, and for a run, clicking a step inside opens that step's output or reasoning. Failures and running calls keep a visible pill or spinner on the folded line, and every other tool keeps its card. Folding reasoning with the calls it reasons about is what keeps a turn from carrying a `Thinking` line above every group; the consequence is that a `Thinking` line no longer opens itself while the reasoning streams — the folded line shows a spinner instead.
...
- The box is the only filled shape in the composer: attachment, access/trust, model, thinking, and send/stop share one compact row under the editor, so the composer is two lines, that row and its own padding — measured, 84pt tall.
